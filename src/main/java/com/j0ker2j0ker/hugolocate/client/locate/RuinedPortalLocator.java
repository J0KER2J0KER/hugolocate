...
/**
 * Predicts ruined portal positions from the region seed formula and filters
 * by biome viability via the native cubiomes binding (CubiomesBridge).
 */
public class RuinedPortalLocator {

    private static final long WORLD_SEED = -2394875239847523984L;
    private static final long SALT = 19827364L;

    private static final int REGION_SIZE = 40; // chunks per region
    private static final int CHUNK_RANGE = 25; // sub-region range

    private static final long MASK_48 = (1L << 48) - 1;

    /** Returns {chunkX, chunkZ} of the ruined portal attempt position for the given region. */
    ...
    /** Up to 3 nearest biome-viable predicted ruined portal positions, sorted by distance. */
    public static java.util.List<LocateResult> findNearest(long playerX, long playerZ, int searchRadiusRegions) {
        ...
        for (int dx = -searchRadiusRegions; dx <= searchRadiusRegions; dx++) {
            for (int dz = -searchRadiusRegions; dz <= searchRadiusRegions; dz++) {
                ...
                if (!CubiomesBridge.isViableRuinedPortalPos(WORLD_SEED, (int) blockX, (int) blockZ)) {
                    continue;
                }

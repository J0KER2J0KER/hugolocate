...
/**
 * Predicts Nether Fortress positions. Shares the same salt/placement
 * formula as Bastion - a Fortress generates wherever Bastion is NOT
 * biome-viable.
 */
public class FortressLocator {
    ...
    /** Up to 3 nearest Fortress positions, sorted by distance. */
    public static java.util.List<LocateResult> findNearest(long playerX, long playerZ, int searchRadiusRegions) {
        ...
        for (int dx = -searchRadiusRegions; dx <= searchRadiusRegions; dx++) {
            for (int dz = -searchRadiusRegions; dz <= searchRadiusRegions; dz++) {
                ...
                if (!CubiomesBridge.isViableFortressPos(WORLD_SEED, (int) blockX, (int) blockZ)) {
                    continue;
                }

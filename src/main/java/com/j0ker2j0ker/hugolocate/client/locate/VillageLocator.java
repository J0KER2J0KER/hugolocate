...
/**
 * Predicts village positions from the region seed formula and filters
...
public class VillageLocator {
    ...
    private static final long SALT = 1837462910L;

    private static final int REGION_SIZE = 34; // chunks per region
    private static final int CHUNK_RANGE = 26; // spacing - separation = 34 - 8
    ...
    /** Up to 3 nearest biome-viable predicted village positions, sorted by distance. */
    public static java.util.List<LocateResult> findNearest(long playerX, long playerZ, int searchRadiusRegions) {
        ...
        for (int dx = -searchRadiusRegions; dx <= searchRadiusRegions; dx++) {
            for (int dz = -searchRadiusRegions; dz <= searchRadiusRegions; dz++) {
                ...
                if (!CubiomesBridge.isViableVillagePos(WORLD_SEED, (int) blockX, (int) blockZ)) {
                    continue;
                }
